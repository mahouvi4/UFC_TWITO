                  ...
                  <div class="row">
                          ...
                          <div class="card su_reponx " id="{{$item->id}}">
                                ...
                                <div class="group-control">
                                    <details> <summary style="color:#ed0a70">{{$item->count_responx}} <small style="color:#1bffff">Sub-Comentario(s)</small></summary><textarea id="{{$item->id}}" style="border-radius:9px;width:100%" name="reponse" class="reponxi"></textarea><a href="#" class="btn btn"  style="border:none;border-radius:20px;width:50%;background:linear-gradient(-100deg,rgb(1, 16, 22), #1bffff);margin-left:75px;" onclick="ekox()">Responder</a>
                                    
                                      
                                          <div id="fom" class="fmo">
                                                    
                                                        @foreach($item->responsive as $oo)
                                                        <br></br>
                                                     
                                                           <form>
                                                               <fieldset  class="ml-5" style="border: thin silver solid;border-radius:12px">
                                                                          @if($oo->id_prof == NULL) 
                                                                              <legend><img src="{{asset('user/'.$oo->user->photo)}}"  style="width:30px;height:30px;border-radius:35px;margin-top:-16px;margin-left:16px"></legend>
                                                                              <small><marquee style="font-family:cambria;color:rgb(218, 247, 166);background:linear-gradient(-100deg, #1bffff,rgb(1, 16, 22))">{{$oo->profile.' '.$oo->namo}} of {{$oo->user->ufc->name_ufc}}🐦</marquee></small><br>
                                                                              @else

                                                                            <legend><img src="{{asset('prof/'.$oo->prof->photo)}}"  style="width:30px;height:30px;border-radius:35px;margin-top:-16px;margin-left:16px"></legend>
                                                                            <small><marquee style="font-family:cambria;color:rgb(218, 247, 166);background:linear-gradient(-100deg, #1bffff,rgb(1, 16, 22))">{{$oo->profile.' '.$oo->namo}}🐦</marquee></small><br>
                                                                          @endif



                                                                         
                                                                         <small style="font-family:cambria;color:rgb(218, 247, 166);margin:12px">{{$oo->reponse}}</small><br>
                                                                         <small style="font-family:cambria;color:rgb(218, 247, 166);text-align:end;background:linear-gradient(200deg, #1bffff,rgb(66, 6, 49))">{{$oo->created_at}}</small>
